...
Please answer the question based on the webpage content. If the answer is not in the content, say so clearly.`;
        ...
        if (settings.enableStreaming) {
            ...
            chrome.runtime.sendMessage({
                ...
                if (!response) {
                    console.error('No response received from background script');
                    appendMessage('system', 'Error: No response received from the server.', new Date().toLocaleTimeString());
                    return;
                }
                ...
                if (response.success) {
                    ...
                    if (response.data && response.data.status === 200) {
                        ...
                        if (response.data.data && response.data.data.response) {
                            ...
                            console.error('Response data is missing expected format:', response.data);
                            appendMessage('system', 'Error: Received malformed response from the server.', new Date().toLocaleTimeString());
                            return;
                        }
                        
                        if (!aiResponse) {
                            console.error('Empty AI response after processing');
                            appendMessage('system', 'Error: Received empty response from the server.', new Date().toLocaleTimeString());
                            return;
                        }
                        console.log('AI response received, length:', aiResponse.length);
